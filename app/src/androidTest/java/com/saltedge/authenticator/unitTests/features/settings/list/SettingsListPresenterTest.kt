 ...
 */
package com.saltedge.authenticator.unitTests.features.settings.list
...
class SettingsListPresenterTest {
    ...
    fun onListItemClickTest_fingerprint() {
        val presenter = createPresenter(viewContract = null)
        Mockito.doReturn(false).`when`(mockBiometricTools).isFingerprintNotConfigured(TestTools.applicationContext)
        presenter.onListItemClick(itemId = R.string.settings_fingerprint)

        Mockito.verifyNoMoreInteractions(mockView)

        Mockito.doReturn(true).`when`(mockBiometricTools).isFingerprintNotConfigured(TestTools.applicationContext)
        presenter.onListItemClick(itemId = R.string.settings_fingerprint)
