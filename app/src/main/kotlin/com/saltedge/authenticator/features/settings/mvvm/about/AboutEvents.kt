...
class LicensesFragmentEvent : ViewModelEvent() {

    override fun handle(fragment: AboutFragment) {
        super.handle(fragment)
        fragment.activity?.addFragment(LicensesFragment())
    }
}

class WebViewFragmentEvent : ViewModelEvent() {

    override fun handle(fragment: AboutFragment) {
        super.handle(fragment)
        fragment.activity?.addFragment(
            WebViewFragment.newInstance(
                TERMS_LINK,
                fragment.getString(R.string.about_terms_service)
            )
        )
    }
}
